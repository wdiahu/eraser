...
local void treat_file(iname) char *iname;
{
    ...
    if (verbose)
    {
        fprintf(stderr, "%s:\t%s", ifname, (int)strlen(ifname) >= 15 ? "" : ((int)strlen(ifname) >= 7 ? "\t" : "\t\t"));
    }
 ...
 */
local void do_list(ifd, method) int ifd; /* input file descriptor */
 ...
 */
local void treat_dir(dir) char *dir;
{
    dir_type *dp;
    DIR *dirp;
